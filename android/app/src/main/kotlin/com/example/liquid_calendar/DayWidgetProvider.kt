...
class DayWidgetProvider : AppWidgetProvider() {
    ...
    private fun updateWidget(context: Context, manager: AppWidgetManager, id: Int) {
        ...
        // Adaptive font sizing based on widget size
        val options = manager.getAppWidgetOptions(id)
        val minWidth = options.getInt(AppWidgetManager.OPTION_APPWIDGET_MIN_WIDTH)
        val minHeight = options.getInt(AppWidgetManager.OPTION_APPWIDGET_MIN_HEIGHT)

        // Scale fonts: smaller widget → smaller text
        val scale = when {
            minWidth < 180 || minHeight < 180 -> 0.6f  // ~2x2
            minWidth < 250 || minHeight < 250 -> 0.8f  // ~3x2
            else -> 1.0f                                // full size
        }

        val weekdaySize = (20 * scale).toInt().coerceAtLeast(12)
        val daySize = (40 * scale).toInt().coerceAtLeast(20)

        views.setTextViewTextSize(R.id.widget_weekday, android.util.TypedValue.COMPLEX_UNIT_SP, weekdaySize.toFloat())
        views.setTextViewTextSize(R.id.widget_day, android.util.TypedValue.COMPLEX_UNIT_SP, daySize.toFloat())
